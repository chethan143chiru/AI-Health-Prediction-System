...
import { DISEASE_CLASSES, SYMPTOM_FEATURES } from './schema';

// Definitive symptom profiles for all 41 diseases based on the Kaggle / Columbia University Medical Dataset
export const DISEASE_SYMPTOM_MAP: Record<string, string[]> = {
  ...
  "Tuberculosis": ["chills", "vomiting", "fatigue", "weight_loss", "cough", "high_fever", "breathlessness", "sweating", "loss_of_appetite", "mild_fever", "phlegm", "chest_pain", "blood_in_sputum", "swelled_lymph_nodes", "malaise"],
...
export function generateCSVDataset(): string {
  const header = [...SYMPTOM_FEATURES, "prognosis"].join(",");
  const rows: string[] = [header];

  // 120 samples per disease = 4,920 total training records
  for (const disease of DISEASE_CLASSES) {
    const coreSymptoms = DISEASE_SYMPTOM_MAP[disease] || [];
    
    for (let sample = 0; sample < 120; sample++) {
      // Deterministic PRNG seed per sample for full reproducibility
      const featureValues = SYMPTOM_FEATURES.map((feature, fIdx) => {
        const isCore = coreSymptoms.includes(feature);
        if (isCore) {
          // Core symptoms are present 85% to 100% of the time
          const dropProbability = ((sample * 17 + fIdx * 31) % 100) < 15;
          return dropProbability ? 0 : 1;
        } else {
          // Noise symptoms present < 1.5% of the time
          const noise = ((sample * 13 + fIdx * 97) % 1000) < 12;
          return noise ? 1 : 0;
        }
      });

      // Ensure at least 2 symptoms are present
      if (featureValues.reduce((a, b) => a + b, 0) < 2) {
        for (const s of coreSymptoms.slice(0, 2)) {
          const idx = SYMPTOM_FEATURES.indexOf(s);
          if (idx !== -1) featureValues[idx] = 1;
        }
      }

      rows.push([...featureValues, disease].join(","));
    }
  }
